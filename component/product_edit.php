...
// Fetch image from product_image table
...
if ($imageResult && mysqli_num_rows($imageResult) > 0) {
    ...
    $image_path = "../assets/no-image.jpg";  // Default image
}
$status = $row["status"];
$brand = $row["brand"];
...
if (isset($_POST['edit'])) {
    ...
    $name = mysqli_real_escape_string($connect, trim($_POST['product_name']));
    $description = mysqli_real_escape_string($connect, trim($_POST['product_description']));
    $category = mysqli_real_escape_string($connect, trim($_POST['category']));
    $price = trim($_POST['product_price']);
    $brand = mysqli_real_escape_string($connect, trim($_POST['product_brand']));
    $quantity = intval($_POST['quantity']);
    $color = mysqli_real_escape_string($connect, trim($_POST['product_color']));
    $condition = mysqli_real_escape_string($connect, trim($_POST['product_condition']));

    // ✅ SERVER-SIDE VALIDATION
    $errors = [];

    if (!preg_match("/^[A-Za-z\s]{1,50}$/", $name)) {
        $errors[] = "Product name should contain only letters (max 50 chars).";
    }
    if (strlen($description) > 300 || strlen($description) < 10) {
        $errors[] = "Description must be 10–300 characters long.";
    }
    if (!preg_match("/^[0-9]+(\.[0-9]{1,2})?$/", $price)) {
        $errors[] = "Price must be a valid number.";
    }
    if (!preg_match("/^[A-Za-z\s]{0,30}$/", $brand) && $brand !== '') {
        $errors[] = "Brand must contain only letters (max 30 chars).";
    }
    if (!preg_match("/^[A-Za-z\s]{0,30}$/", $color) && $color !== '') {
        $errors[] = "Color must contain only letters (max 30 chars).";
    }
    if ($quantity < 0) {
    ...
    if (!empty($errors)) {
        ...
        if (!empty($_FILES['product_image']['name'])) {
            ...
            if (in_array($fileType, $allowedTypes)) {
                ...
                echo "<script>alert('Invalid image format. Only JPG, PNG, WEBP, or GIF allowed.');</script>";
            }
        }
